...
apt -y install pkg-config autoconf autoconf-archive libtool automake build-essential python3-future python-future libssl-dev 
...
./autogen.sh && ./configure CFLAGS='-g -O2' --sysconfdir=/etc --localstatedir=/var --libdir=/usr/local/lib64 --prefix=/usr/local || exit 1
...
fpm -a ${PACKAGE_ARCH} -s dir -t deb -n ${PACKAGE_NAME} -v ${VERSION//v} -C ${TMPDIR} -p ${PACKAGE_NAME}_VERSION_ARCH.deb || exit 1
